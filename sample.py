''' Sample
   ...
   #   inter bw same classes, Bigger size 256 
import functools
import math
import numpy as np
...
def run(config):
  ...
  # Sample a number of images and stick them in memory, for use with TF-Inception official_IS and official_FID  ## size = 64, 256, 128
  ...
  if config['sample_sheets']:
    ...
    utils.sample_sheet(G, classes_per_sheet=utils.classes_per_sheet_dict[config['dataset']], 
                         ...
                         z_=z_,)
  # Classwise img generation for each class
  if config['sample_classwise']:
    print('Preparing conditional sample ...')
    folder_number=config['sample_sheet_folder_num']
    if folder_number == -1:
      folder_number = config['load_weights']
      # for the broken class
    for i in range(20000): # 1497

      utils.sample_classwise(G, classes_per_sheet=utils.classes_per_sheet_dict[config['dataset']], 
                           num_classes=config['n_classes'], 
                           samples_per_class=1, parallel=config['parallel'],
                           samples_root=config['samples_root'], 
                           experiment_name=experiment_name,
                           folder_number=folder_number,img_class='broken',img_no=i,img_class_no=0,
                           z_=z_,)
      # for the discolored class
    for i in range(20000): #  4153
      ...
      utils.sample_classwise(G, classes_per_sheet=utils.classes_per_sheet_dict[config['dataset']], 
                           ...
                           samples_root=config['samples_root'], 
                           experiment_name=experiment_name,
                           folder_number=folder_number,img_class='discolored',img_no=i,img_class_no=1,
                           z_=z_,)
      # for the pure class
    for i in range(20000): # 0
      ...
      utils.sample_classwise(G, classes_per_sheet=utils.classes_per_sheet_dict[config['dataset']], 
                           ...
                           samples_root=config['samples_root'], 
                           experiment_name=experiment_name,
                           folder_number=folder_number,img_class='pure',img_no=i,img_class_no=2,
                           z_=z_,)
      # for the silkcut class
    for i in range(20000): # 5516
      ...
      utils.sample_classwise(G, classes_per_sheet=utils.classes_per_sheet_dict[config['dataset']], 
                           ...
                           samples_root=config['samples_root'], 
                           experiment_name=experiment_name,
                           folder_number=folder_number,img_class='silkcut',img_no=i,img_class_no=3,
                           z_=z_,)
